...
fn main() {
    ...
    let map = Generator::new().room_size((4,4), (16,16)).size(128, 128).iterations(128).corridor_errantness(0.75).generate();
    ...
    for (cell, x, y) in map.cell_matrix.iter_enumerate() {
        match cell.kind {
            CellKind::Room(_) => img.set_pixel(x.into(), y.into(), room_pixel),
